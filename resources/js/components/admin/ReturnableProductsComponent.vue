<template>
    <div>
        ...
        <!-- Remove Stock Modal - similar structure to Add Stock Modal -->
        <div
            v-if="showRemoveStockModal"
            class="modal-overlay"
            @click.self="closeRemoveStockModal"
        >
            <div class="modal-container">
                <div class="modal-header">
                    <h5>Retirer du Stock</h5>
                    <button @click="closeRemoveStockModal" class="modal-close">
                        &times;
                    </button>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label>Produit</label>
                        <select
                            v-model="removeStock.product_id"
                            class="form-control"
                        >
                            <option value="">Sélectionner un produit</option>
                            <option
                    ...
                    <div class="form-group">
                        <label>Quantité</label>
                        <input
                            v-model.number="removeStock.quantity"
                            type="number"
                            min="0"
                            step="0.01"
                            class="form-control"
                ...
                <div class="modal-footer">
                    <button @click="closeRemoveStockModal" class="btn-cancel">
                        Annuler
                    </button>
                    <button @click="removeStockItem" class="btn-submit">
                        Retirer
                    </button>
                </div>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            generateAllDepositsPrint() {
                ...
                <html>
                <head>
                    <title>Historique des Consignations</title>
                    <style>
                        * {
                            margin: 0;
                            padding: 0;
            ...
            removeStockItem() {
                if (!this.removeStock.product_id) {
                    alert('Veuillez sélectionner un produit');
                    return;
                }
                if (this.removeStock.quantity <= 0) {
                    alert('La quantité doit être supérieure à 0');
                    return;
                }
                ...
                axios
                    .post('/deposits/remove', {
                        ...
                        quantity: this.removeStock.quantity,
                        comment: this.removeStock.comment || 'Retrait de stock',
                    })
                    .then((response) => {
